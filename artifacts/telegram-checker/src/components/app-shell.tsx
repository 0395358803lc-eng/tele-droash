    ...
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[238px] flex-col bg-[hsl(var(--sidebar))] text-[hsl(var(--sidebar-foreground))] md:flex">
      <div className="flex h-[74px] items-center border-b border-[hsl(var(--sidebar-border))] px-6">
        <Link href="/" className="flex items-center gap-3" data-testid="link-brand">
          ...
          <div><div className="font-display text-[15px] font-bold tracking-tight text-[hsl(var(--sidebar-accent-foreground))]">Relaycheck</div><div className="font-mono text-[9px] uppercase tracking-[.17em] text-[hsl(var(--sidebar-foreground)/.5)]">telegram operations</div></div>
        </Link>
      </div>
      ...
      <div className="mt-auto px-4 pb-5">
        <div className="mb-4 rounded-md border border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar-accent)/.55)] p-3.5">
          <div className="mb-2 flex items-center gap-2 text-[11px] font-semibold text-[hsl(var(--sidebar-accent-foreground))]"><ShieldCheck size={14} className="text-[hsl(var(--sidebar-primary))]" /> Vùng an toàn</div>
           <p className="text-[11px] leading-relaxed text-[hsl(var(--sidebar-foreground)/.58)]">Phiên Telegram được lưu bảo mật phía máy chủ.</p>
    ...
    <div className="md:pl-[238px]">
      <header className="sticky top-0 z-20 flex h-[74px] items-center justify-between border-b border-[hsl(var(--border))] bg-[hsl(var(--background)/.9)] px-5 backdrop-blur-md sm:px-8">
        <div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-md bg-[hsl(var(--sidebar))] text-[hsl(var(--sidebar-primary))] md:hidden"><TerminalSquare size={16} /></div><div><div className="font-mono text-[10px] uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">Relaycheck / {pageLabel.toLowerCase()}</div><div className="font-display text-lg font-semibold tracking-tight">{pageLabel}</div></div></div>
         <div className="flex items-center gap-3"><span className="hidden items-center gap-2 font-mono text-[10px] uppercase tracking-wider text-[hsl(var(--muted-foreground))] sm:flex"><span className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--primary))]" /> Dữ liệu Telegram thật</span><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[hsl(var(--sidebar))] font-display text-xs font-bold text-[hsl(var(--sidebar-primary))]" data-testid="avatar-operator">OP</div></div>
      </header>
